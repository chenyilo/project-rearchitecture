...
> 概念在迭代中漂移，命名和实际语义对不上。这里**先从代码逆向出真实存在的领域模型**，
...
| `validate()` 里在写数据库；`Utils` 里装核心业务规则 | D3 名实不符检测 → 先抽副作用再改名 |
| 有完整实现但业务上不存在的旧流程残留 | D4 幽灵概念检测 → 取证后删除 |
| 业务规则散在 5 个文件里，没有载体 | D5 缺失概念检测 → 引入抽象 |
| 项目没有测试，不敢动 | 行为冻结：特征化测试 + 黄金样本 + 契约快照 |
| 怀疑有死代码，但不敢删 | 两条取证路线（运行时 / 入口可达性）+ T1–T6 分类 |
...
│   ├── dead-code-removal.md          死代码判定与删除（两条取证路线、T1–T6）
│   ├── process.md                    通用重构流程
│   ├── safety.md                     安全协议 §1–§9
│   ├── smells.md                     坏味道清单 家族 1–7
│   ├── language-profiles.md          语言惯用法与测试命令
│   └── catalog-*.md                  7 份重构手法目录
└── templates/
    ...
    ├── RENAME-MAP-template.md         改名映射与批次
    ├── MIGRATION-PLAN-template.md     迁移方案（四件套）
    ├── DEAD-CODE-CANDIDATES-template.md 死代码候选与取证记录
    └── REARCH-STATE-template.md       跨会话状态
```

`process.md`、`safety.md`、`smells.md`、`language-profiles.md` 与 `catalog-*.md`
是通用重构规则与手法，本仓库内自带一份，安装后不需要额外依赖。
...
> 「把这段代码清理一下」不属于这里的触发词。触发点必然是**项目级**或**语义级**的。
...
9. **删代码要取证而非搜索** —— `grep` 零命中不构成删除依据；找不到触发路径
   只能说明它是候选，不能说明它是死的。纯本地模式下必须收窄删除范围。

---

## 贡献

欢迎提交 Issue 与 PR。改进建议请附带一个真实项目中的反例——这里的方法论都来自真实的重架构失败模式。
